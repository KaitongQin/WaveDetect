...
class AIHumanFunctionModel:
    def __init__(self, model_name_or_path, ntrain, rep_token, batch_size, random_seed=2025, ai_weight=1, human_weight=1, n_difference=1, direction_method='pca'):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map="cuda:2")
    ...
    def process_data(self, data, mode="train"):
        ...
        for item in data:
            if item["direct_prompt"].strip():
                input_statements.append(item["direct_prompt"])
                input_labels.append(1)
            if item["human_text"].strip():
                input_statements.append(item["human_text"])
                input_labels.append(0)
