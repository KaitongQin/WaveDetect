...
def add_mean_score(data, json_data, score_key="rep_reader_scores_dict", out_key="score"):
    """
    Add to each item:
        item[out_key] = mean(item[score_key])
    """
    for data_item, json_item in zip(data, json_data):
        # Only perform calculation and writing when the target score key exists in the json_data item
        if score_key in json_item:
            # Calculate the mean and convert to float, write to the corresponding item in data
            mean_score = float(np.mean(json_item[score_key]))
            data_item[out_key] = mean_score
    return data
...
def entrance(args):
    ...
    if args.bootstrap_iter == -1:
        ...
        for test_data_path in test_data_paths:
            ...
            train_json_data_saved = add_mean_score(train_data, train_json_data)
            test_json_data_saved = add_mean_score(test_data, test_json_data)
            with open(f'results/score_results/{result_file_name}_test_score.json', 'w') as json_file:
                json.dump(test_json_data_saved, json_file, indent=4)
            with open(f'results/score_results/{result_file_name}_train_score.json', 'w') as json_file:
                json.dump(train_json_data_saved, json_file, indent=4)
